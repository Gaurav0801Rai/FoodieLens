...
**Test the API (PowerShell, Windows / Cursor terminal)**

With the API running on port 8080 (`python -m uvicorn api.main:app --reload --port 8080`):
...
# Generate recommendations (POST + JSON body)
$body = @'
{
...
$response = Invoke-WebRequest `
  -Uri "http://127.0.0.1:8080/api/v1/recommendations" `
  -Method POST `
...
> On Windows, `curl` is an alias for `Invoke-WebRequest` and does not support Unix flags (`-X`, `-H`, `-d`). Use the commands above instead.

Or run the helper script: `.\scripts\test-recommendations.ps1`
