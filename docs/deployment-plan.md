...
**Local (PowerShell / Cursor terminal on Windows)**

```powershell
$base = "http://127.0.0.1:8080"   # or your Railway HTTPS URL

Invoke-WebRequest -Uri "$base/api/v1/health" -UseBasicParsing |
  Select-Object -ExpandProperty Content

Invoke-WebRequest -Uri "$base/api/v1/metadata/locations" -UseBasicParsing |
  Select-Object -ExpandProperty Content

.\scripts\test-recommendations.ps1   # POST /api/v1/recommendations
```

**Production (bash / Git Bash / WSL)**
...
| Variable | Required | Default | Purpose |
|----------|----------|---------|---------|
| `NEXT_PUBLIC_API_URL` | **Yes** (prod) | `http://localhost:8080` | Railway API base URL |
...
| Area dropdown empty | API 503 or CORS blocked | Check `/api/v1/metadata/locations` (see §1.5) |
