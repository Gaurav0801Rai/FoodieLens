...
Test recommendations (PowerShell, from project root):
    .\\scripts\\test-recommendations.ps1
"""
...
# Load environment variables from project-root .env before any app imports.
...
def create_recommendations(body: RecommendationRequest) -> RecommendationResponseOut:
    ...
    if not settings.has_llm_api_key:
        raise HTTPException(
            status_code=503,
            detail="LLM API key not configured. Set LLM_API_KEY or GROQ_API_KEY in .env.",
        )
